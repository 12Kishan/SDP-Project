      ...
      {!data?.user.isAdmin && (
        <>
          {userMenus.map((item) => (
            <li
              key={item.title}
              className={`gap-y-2 ${item.gap ? "mt-8" : ""}`}
            >
              <Link href={item.link}>
                <span
                  className={`hover:bg-gray-700 mb-3 hover:rounded-md w-full text-sm whitespace-pre flex items-center ${
                    item.link === pathname ? "bg-gray-700 rounded-md" : ""
                  }`}
                >
                  <div className={`p-2`}>
                    <span>{item.icon}</span>
                  ...
                  <span
                    className={`mx-4 text-xs md:text-base lg:text-base text-white  whitespace-pre ${
                      isOpen
                        ? "lg:duration-500 lg:ease-in lg:h-auto lg:opacity-100 lg:transition-all lg:w-auto"
                        : "lg:duration-200 lg:ease-out lg:invisible lg:opacity-0 lg:transition-all"
                    }`}
                  >
                    {item.title}
                  </span>
...
{data?.user.isAdmin && (
      ...
      <li>
        <button
          className="hover:bg-gray-700 p-2 mt-8 mb-3 hover:rounded-md w-full text-sm whitespace-pre flex items-center"
          onClick={() => {
