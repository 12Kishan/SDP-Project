...
const StatisticPage = async ({ params }: Props) => {
    ...
    return (
        <>
            <section className='h-full bg-slate-500'>
                <div className="flex items-center justify-center px-4 sm:px-6 py-6 lg:px-8 lg:py-12">
                    <div className="bg-gray-900 w-full mx-28 rounded-3xl px-7 py-7" >
                        ...
                        <div className="bg-white py-2 px-5 rounded-md mt-5">
                            <div className="font-bold text-gray-900 mb-2 text-sm md:text-lg lg:text-xl">{user.name}</div>
                            <hr className="h-px mb-2 bg-gray-500 border-1 border-gray-500 dark:bg-gray-700" />
                            <div className="items-center text-xs md:text-sm lg:text-base lg:flex lg:justify-between lg:items-center md:flex md:justify-between font-semibold md:items-center">
                                <div className="flex items-center gap-x-1 justify-center">
                                    <div>Type</div>
                                    <div className="px-1 py-0.5 font-bold bg-slate-400 rounded-md mb-0.5">{questions[0].questionType}</div>
                                </div>
                                <div className="flex items-center gap-x-1 justify-center">
                                    <div>Topic</div>
                                    <div className="px-1 py-0.5 font-bold bg-slate-400 rounded-md mb-0.5">{quiz.topic}</div>
                                </div>
                                <div className="flex items-center gap-x-1 justify-center">
                                    <div>Difficulty</div>
                                    <div className="px-1 py-0.5 font-bold bg-slate-400 rounded-md mb-0.5">{quiz.difficulty}</div>
                                </div>
                                <div className="flex items-center gap-x-1 justify-center">
                                    <div>No. of questions</div>
                                    <div className="px-1 py-0.5 font-bold bg-slate-400 rounded-md">{questions.length}</div>
                                </div>
                            </div>
                        </div>
                        <div className="lg:grid lg:grid-row-2 lg:grid-flow-col lg:gap-0.5">

                            <div className="lg:col-span-2 bg-white py-2 px-5 rounded-md mt-5 items-center text-center text-xs md:text-sm lg:text-base lg:flex lg:justify-between lg:items-center md:flex md:justify-between font-semibold md:items-center">
                                <Accuracy correct={correct} incorrect={questions.length - correct} />
                            </div>
                            <div className="lg:col-span-2 bg-white py-2 px-5 rounded-md mt-5 items-center text-center text-xs md:text-sm lg:text-base lg:flex lg:justify-between lg:items-center md:flex md:justify-between font-semibold md:items-center">
                                <TimeTaken timeTaken={quiz.timeTaken} />
                            </div>
                            <div className="lg:row-span-2 flex h-56 justify-center mt-5 rounded-md bg-white lg:ml-4">
                                <UserChart correct={correct} incorrect={questions.length - correct} />
                            </div>
                        </div>
                        <div className="bg-white flex items-center justify-center py-2 px-5 rounded-md mt-5">
